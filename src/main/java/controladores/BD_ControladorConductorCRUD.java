...
public class BD_ControladorConductorCRUD {
	...
	public void modifyUsuario(Conductor user) {
		this.em = entityManagerFactory.createEntityManager();
	...
	public void createUsuario(Conductor user) {
		this.em = entityManagerFactory.createEntityManager();
		// En este caso es necesario iniciar una transacción en la base de datos
		// porque vamos a persistir información en la misma
		this.em.getTransaction().begin();
	...
	public Conductor findByPK(int codconductor) {
		this.em = entityManagerFactory.createEntityManager();
		Conductor aux = null;
		// Se crea el objeto Query a partir de una SQL nativa
		this.consulta = em.createNativeQuery("Select * from conductor where codconductor = ?", Conductor.class);
		this.consulta.setParameter(1, codconductor);
		aux = (Conductor) consulta.getSingleResult();
		this.em.close();
		return aux;

	}

	public Conductor findByCodConductor(int codconductor) {
		this.em = entityManagerFactory.createEntityManager();
		this.consulta = em.createNamedQuery("Conductor.findCodConductor");
		this.consulta.setParameter("codconductor", codconductor);
		Conductor user = (Conductor) consulta.getSingleResult();
		this.em.close();
		return user;
	}
	...
	public List<Conductor> findAll() {
		this.em = entityManagerFactory.createEntityManager();
		this.consulta = em.createNamedQuery("Conductor.findAll");
		List<Conductor> listaUsuarios = (List<Conductor>) consulta.getResultList();
		this.em.close();
	...
	public List<Conductor> findByNumSeg(String numSeg) {
		this.em = entityManagerFactory.createEntityManager();
		this.consulta = em.createNativeQuery("Select * from conductor where numSegSocial=?", Conductor.class);
		this.consulta.setParameter(1, numSeg);
		List<Conductor> listaUsuarios = (List<Conductor>) consulta.getResultList();
		this.em.close();
		return listaUsuarios;
	}
